...
if len(sys.argv) < 2:
    ...
    print(
        "    The environment variable COMPDIR can be set to the root of RunestoneComponents"
    )
    print(
        "    Otherwise you are best off running this command from the root of the RunestoneComponents distribution"
    )
    sys.exit(1)
...
if "COMPDIR" in os.environ:
    ...
    print(
        "Warning: No COMPDIR environment variable set using . for RunestoneComponents"
    )
    data_path = "runestone/dist"

data = json.loads(open(os.path.join(data_path, "webpack_static_imports.json")).read())
